...
def with_edge_vectors(data: Type, with_lengths: bool = True) -> Type:
    ...
    if _keys.EDGE_VECTORS_KEY not in data:
        ...
        edge_vec = pos[edge_index_trg] - pos[edge_index_src]
        ...
        if _keys.CELL_KEY in data and _keys.EDGE_CELL_SHIFT_KEY in data:
            ...
            edge_cell_shift = data[_keys.EDGE_CELL_SHIFT_KEY].to(dtype=pos.dtype)
            if cell.shape[0] > 1:
                batch = data[_keys.BATCH_KEY]
                # Cell has a batch dimension
                # note the ASE cell vectors as rows convention
                edge_vec = edge_vec + torch.einsum(
                    "ni,nij->nj", edge_cell_shift, cell[batch[edge_index_src]]
                )
