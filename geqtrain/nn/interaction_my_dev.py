...
from geqtrain.nn.mace.irreps_tools import reshape_irreps, inverse_reshape_irreps, equivariant_batch_norm
...
class InteractionModule(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.env_linears_BN = torch.nn.ModuleList([])
        self.tp_1_BN = torch.nn.ModuleList([])
        self.tp_2_BN = torch.nn.ModuleList([])
        self.last_linear_BN = torch.nn.ModuleList([])
        ...
        for layer_idx, (l_arg_irreps, l_out_irreps) in enumerate(
            ...
            self.env_linears_BN.append(
                equivariant_batch_norm(env_embed_irreps)
            )
            ...
            self.tp_1_BN.append(
                equivariant_batch_norm(o3.Irreps([(env_embed_multiplicity, ir) for _, ir in l_arg_irreps]))
            )
            self.tp_2_BN.append(
                equivariant_batch_norm(o3.Irreps([(env_embed_multiplicity, ir) for _, ir in env_embed_irreps]))
            )
            ...
            self.last_linear_BN.append(equivariant_batch_norm(full_out_irreps))
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        for (latent,
            ...
            local_env_per_active_atom = env_lin_bn(local_env_per_node[active_node_centers])
            local_env_per_active_atom = env_linear(local_env_per_active_atom)
            ...
            features = tp_1_BN(features)
            local_env_per_active_edge = tp_2_BN(local_env_per_active_edge)
        ...
        out_features[active_edges, :self.out_multiplicity * features_n_scalars] = scalars.to(self.DTYPE)
