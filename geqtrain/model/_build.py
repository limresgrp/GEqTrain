...
def _get_initial_irreps(config: Config, dataset: Optional[AtomicDataset]) -> Dict:
    """Get the initial `irreps_in` for a model from config or dataset."""
    if "irreps_in" in config:
        return config["irreps_in"]
    else:
        return {}

def _build_sequentially(
    builders: list,
    prms: list,
    weights_prms: list,
    config: Config,
    initialize: bool,
    dataset: Optional[AtomicDataset],
    deploy: bool,
    dry_run: bool = False,
) -> Tuple[Optional[Module], Dict, set, set]:
    """Helper to build model sequentially, with a dry run mode."""
    model = None
    weights_to_drop_from_model_state: set[str] = set()
    ...
    for builder, _prms, _w_prm in zip(builders, prms, weights_prms):
        ...
        if "initialize" in pnames: params["initialize"] = initialize
        if "deploy" in pnames: params["deploy"] = deploy
        if "config" in pnames: params["config"] = config
        if "dataset" in pnames: params["dataset"] = dataset
        if "model" in pnames: params["model"] = model
        
        # Temporarily set the dry_run_mode in the config so `instantiate` can see it
        config["_dry_run_mode"] = dry_run

        model: Module = builder(**params)

        if isinstance(model, Module) and getattr(model, "_is_wrapper", False):
            continue
        ...
        if _w_prm is None:
            ...
            for n, p in model.named_parameters():
                if n in params_just_added:
                    if _w_prm == "freeze": p.requires_grad = False
                    elif _w_prm == "tune": add_tags_to_parameter(p, "tune")
                    else: assert _w_prm == "load"

        weights_already_loaded.update(current_model_param_names)

    # Clean up the temporary key
    config.pop("_dry_run_mode", None)

    return model, config, weights_to_drop_from_model_state, weights_already_loaded
...
def model_from_config(
    ...
    # checks
    if not builders:
        raise ValueError("`model_builders` cannot be empty.")

    if config.get("dry_run", False):
        # == Pass 1: Dry run to determine full irreps ==
        # We need to deepcopy the config to avoid side effects between runs
        dry_run_config = copy.deepcopy(config)
        dry_run_config["irreps_in"] = _get_initial_irreps(dry_run_config, dataset)
        
        dry_run_model, _, _, _ = _build_sequentially(
            builders=builders, prms=prms, weights_prms=weights_prms,
            config=dry_run_config, initialize=False, dataset=None, deploy=True,
            dry_run=True
        )
        
        # The full set of irreps is the input to the first module of the dry_run_model
        final_irreps_in = dry_run_model.irreps_out
        config["irreps_in"] = final_irreps_in

    # == Pass 2: Final build with correct irreps ==
    ...
    model, config, weights_to_drop_from_model_state, _ = _build_sequentially(
        builders=builders, prms=prms, weights_prms=weights_prms,
        config=config, initialize=initialize, dataset=dataset, deploy=deploy,
        dry_run=False
    )
