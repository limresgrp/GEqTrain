...
def is_key_for_loss_nan(data, loss_func, key):
    '''
    checks whether the loss for the given key must be checked for nans
    data:<class 'geqtrain.data.AtomicData.AtomicData'>
    loss_func:<class 'geqtrain.train.loss.Loss'>
    key: str

    returns key without any _i suffix if key must be checked, None otherwise
    '''
    if hasattr(loss_func.funcs[key], "ignore_nan") and loss_func.funcs[key].ignore_nan:
        key_clean = loss_func.remove_suffix(key)
        if key_clean not in _NODE_FIELDS+_GRAPH_FIELDS:
            return None
        if key_clean not in data:
            return None
        return key_clean
    return None
...
def remove_node_centers_for_NaN_targets_and_edges(
    ...
    if loss_func is not None:
        for key in loss_func.keys:
            key_clean = is_key_for_loss_nan(data, loss_func, key)
            if key_clean is not None:
                if keep_node_types is not None:
                    remove_node_types_mask = ~get_node_types_mask(
                        node_types, keep_node_types, data)
                    data[key_clean][remove_node_types_mask] = torch.nan
                val: torch.Tensor = data[key_clean]
                if val.dim() == 1:
                    val = val.reshape(len(val), -1)

                not_nan_edge_filter = torch.isin(data[AtomicDataDict.EDGE_INDEX_KEY][0], torch.argwhere(
                    torch.any(~torch.isnan(val), dim=-1)).flatten())
    ...
    if exclude_node_types_from_edges is not None:
        exclude_node_types_from_edges_mask = get_node_types_mask(
            node_types, exclude_node_types_from_edges, data)
        keep_edges_filter = ~torch.isin(data[AtomicDataDict.EDGE_INDEX_KEY][1], torch.nonzero(
            exclude_node_types_from_edges_mask).flatten())
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.debug:
            self.log_updates = output.open_logfile(
                "log_updates", propagate=False)
        ...
        if self.keep_type_names is not None:
            self.keep_node_types = find_matching_indices(
                self.type_names, self.keep_type_names)
        if self.keep_node_types is not None:
            self.keep_node_types = torch.as_tensor(
                self.keep_node_types, device=self.torch_device)

        # --- exclude edges from center node to specified node types
        if self.exclude_type_names_from_edges is not None:
            self.exclude_node_types_from_edges = torch.tensor(
                find_matching_indices(self.type_names, exclude_type_names_from_edges))
        if self.exclude_node_types_from_edges is not None:
            self.exclude_node_types_from_edges = torch.as_tensor(
                self.exclude_node_types_from_edges, device=self.torch_device)
        ...
        assert isinstance(n_train, (list, int, type(None))
                          ), "n_train must be of type list, int, or None"
        self.n_train = n_train if isinstance(
            n_train, list) or n_train is None else [n_train]
        assert isinstance(n_val, (list, int, type(None))
                          ), "n_val must be of type list, int, or None"
        self.n_val = n_val if isinstance(
            n_val, list) or n_val is None else [n_val]
        ...
        self._end_of_epoch_callbacks = [load_callable(
            callback) for callback in end_of_epoch_callbacks]
        self._end_of_batch_callbacks = [load_callable(
            callback) for callback in end_of_batch_callbacks]
        self._end_of_train_callbacks = [load_callable(
            callback) for callback in end_of_train_callbacks]
        self._final_callbacks = [load_callable(
            callback) for callback in final_callbacks]

    def _get_num_of_steps_per_epoch(self):
        if hasattr(self, "dl_train"):
            return len(self.dl_train)
        raise ValueError(
            "Missing attribute self.dl_train. Cannot infer number of steps per epoch.")
    ...
    def log_data_points(self, dataset, prefix: str):
        loss_clean_keys = [self.loss.remove_suffix(
            key) for key in self.loss.keys]
        counts = {}
        for data in dataset:
            for loss_clean_key in loss_clean_keys:
                counts[loss_clean_key] = counts.get(
                    loss_clean_key, 0) + torch.sum(~torch.isnan(data[loss_clean_key])).item()
    ...
    def dataset_params(self):
        ...
        elif isinstance(self.dataset_train, ConcatDataset):
            return self.dataset_train.datasets[0].config
        raise ValueError(
            f'Dataset currently used is of type ({type(self.dataset_train)}), which is not supported')
    ...
    def set_dataset(
        ...
        if validation_dataset is None:
            logging.warning(
                "No validation dataset was provided. Using a subset of the train dataset as validation dataset.")

        if self.train_idcs is None or self.val_idcs is None:
            if self.n_train is None:
                if validation_dataset is None:
                    if self.n_val is not None:  # scenario? continue train with same valset but different trainset?
                        self.n_train = [
                            len(ds) - n_valid for ds, n_valid in zip(dataset.datasets, self.n_val)]
                    else:
                        logging.warning(
                            "No 'n_train' nor 'n_valid' parameters were provided. Using default 80-20%")
                        n_total = np.array([len(ds)
                                           for ds in dataset.datasets])
                        ones_mask = n_total == 1
                        n_total[~ones_mask] = (
                            0.8 * n_total[~ones_mask]).astype(int)
                        num_ones = np.sum(ones_mask)
                        ones = np.copy(n_total[ones_mask])
                        ones[np.random.choice(num_ones, int(
                            0.2*num_ones), replace=False)] = 0
            ...
            if self.n_val is None:
                if validation_dataset is not None:
                    ...
                    self.n_val = [
                        len(ds) - n_train for ds, n_train in zip(dataset.datasets, self.n_train)]
            ...
            if validation_dataset is not None:
                for _validation_dataset, n_val in zip(validation_dataset.datasets, self.n_val):
                    ...
                    if self.train_val_split == "random":
                        idcs = torch.randperm(
                            total_n, generator=self.dataset_rng)
        ...
        def select_idcs_from_dset(datasets, idcs):
            if isinstance(datasets, ConcatDataset):
                datasets = datasets.datasets
                indexed_datasets = []
                for _dataset, _idcs in zip(datasets, idcs):
                    # torch_geometric datasets support subsets using `index_select`
                    _dataset = _dataset.index_select(_idcs)
                    _dataset, per_node_outputs_keys = remove_node_centers_for_NaN_targets_and_edges(
                        _dataset, self.loss, self.keep_node_types, self.exclude_node_types_from_edges)
                    if self.per_node_outputs_keys is None:  # this is executed only once
                        self.per_node_outputs_keys = per_node_outputs_keys
                    if _dataset is not None:
                        indexed_datasets.append(_dataset)
                return ConcatDataset(indexed_datasets)
            elif isinstance(datasets, LazyLoadingConcatDataset):
                # this case has less checks, more brittle, to be improved
                #! for now LazyLoadingConcatDataset can work only if BOTH train and val dsets are explicitly provided in yaml
                # todo: make LazyLoadingConcatDataset support automatic 80/20 split
                # datasets.keep_idxs([1]*len(idcs))
                if self.per_node_outputs_keys is None:  # this is executed only once
                    self.per_node_outputs_keys = []
                    if self.loss is not None:
                        for key in self.loss.keys:
                            key_clean = is_key_for_loss_nan(
                                datasets[0], self.loss, key)
                            if key_clean is not None:
                                self.per_node_outputs_keys.append(key_clean)
                return datasets
            raise ValueError(
                f'Passed dataset type ({type(datasets)}) is not supported')

        # build redefined datasets wrt data splitting process above
        self.dataset_train = select_idcs_from_dset(
            dataset, self.train_idcs)
        self.dataset_val = select_idcs_from_dset(
            validation_dataset, self.val_idcs)

        self.logger.info(
            f"Training data structures: {len(self.dataset_train)} | Validation data structures: {len(self.dataset_val)}")
